 ...
 * Twin of `@testa-soft/next`'s `experiments/goal-tracking.ts` (kept per-package
 * so each SDK stays self-contained).
 *
 * Called once per SPA navigation from `initTesta`: arms page_view, click and
 * custom goals for every experiment the visitor is ASSIGNED to (session-live,
 * control included, NOT page-gated — a goal usually completes on a different
 * page than the experiment runs on). Custom goals fire via `pushEvent('name',
 * data)` (also installed as `window.testa.pushEvent` /
 ...
 * SPA nav / unmount before arming the next cycle.
 */
