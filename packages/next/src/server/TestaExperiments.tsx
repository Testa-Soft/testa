/**
 * `<TestaExperiments/>` — the React Server Component wrapper around the client
 ...
 * config-fetch code of its own — just drop `<TestaExperiments projectId=.../>`
 ...
 */
export type TestaExperimentsProps = CommonProps &
  ({ projectId: string; config?: ProjectConfig } | { config: ProjectConfig; projectId?: string });

export async function TestaExperiments(props: TestaExperimentsProps): Promise<JSX.Element | null> {
  const resolved = await resolveConfig(props);
  // Fail open: mirrors marketing-web's `{config && <TestaExperiments/>}` — no
  // config means nothing to apply, so render nothing.
  if (!resolved) return null;

  return (
    <TestaExperimentsClient
      config={resolved}
...
async function resolveConfig(props: TestaExperimentsProps): Promise<ProjectConfig | null> {
