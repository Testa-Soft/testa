 ...
 * boundary validation. Used by the RSC `loadTestaConfig` and the router
 * guard's zero-config mode so both fetch the same way and neither can hang.
 *
...
/**
 * Hard cap on a single config fetch, OFF the request path (RSC loader behind
 * Next's data cache, browser-side guard fetch). Generous vs the CDN's ~50ms.
 */
