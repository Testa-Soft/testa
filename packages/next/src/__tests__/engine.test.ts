...
describe('targeting + exclusions (entry gates only)', () => {
  ...
  it('excludes a FRESH visitor when an exclusion condition matches', () => {
    const config = splitUrlConfig();
    const exp0 = config.experiments[0];
    if (!exp0) throw new Error('fixture must have at least one experiment');
    exp0.exclusions = [{ dimension: 'utm_source', operator: 'contains', value: 'internal' }];
...
describe('cross-domain inbound', () => {
  it('applies a carried assignment for a cross_domain experiment', () => {
    const config = splitUrlConfig();
    const exp1 = config.experiments[0];
    if (!exp1) throw new Error('fixture must have at least one experiment');
    exp1.cross_domain = true;
