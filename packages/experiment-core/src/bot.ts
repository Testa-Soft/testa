 ...
 *
 * Lives here, not in `@testa-soft/next`, because the CLIENT engine needs the
 * same gate: `skipBots` only ever protected the proxy, so a crawler that runs
 * JavaScript (AdsBot-Google renders pages) was bypassed server-side and then
 * bucketed by the client — cookie, exposure and all.
 */
...
const NAMED_CRAWLER_RE =
  /crawler|spider|crawling|slurp|baiduspider|bingpreview|facebookexternalhit|whatsapp|skypeuripreview|screaming frog|bytespider|headlesschrome|phantomjs|lighthouse|pagespeed|pingdom|uptimerobot|statuscake|site24x7|newrelicpinger|mediapartners-google|apis-google|googleother|google-inspectiontool|chatgpt-user|curl\/|wget\/|python-requests|python-urllib|libwww|httpclient|okhttp|go-http-client|node-fetch|axios\/|java\//i;
