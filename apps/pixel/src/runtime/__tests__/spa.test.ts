...
describe('onNavigate — undebounced teardown', () => {
  it('fires synchronously on the nav event, before the debounce', () => {
    const onNavigate = vi.fn();
    const onTransition = vi.fn();
    uninstall = installSpaHandler({ onNavigate, onTransition });

    window.history.replaceState({}, '', '/foo');
    window.dispatchEvent(new CustomEvent(LOCATIONCHANGE_EVENT));

    // Teardown already ran; the re-apply cycle has not.
  ...
  it('does NOT fire on a same-URL pushState (nothing to restore it)', () => {
    const onNavigate = vi.fn();
    const onTransition = vi.fn();
    uninstall = installSpaHandler({ onNavigate, onTransition });

    // Framework updating history state without changing the URL.
  ...
  it('still re-applies when the URL settles back to where it started (A→B→A)', () => {
    const onNavigate = vi.fn();
    const onTransition = vi.fn();
    uninstall = installSpaHandler({ onNavigate, onTransition });
    ...
    window.dispatchEvent(new CustomEvent(LOCATIONCHANGE_EVENT));
    window.history.replaceState({}, '', '/'); // back to the starting URL
    window.dispatchEvent(new CustomEvent(LOCATIONCHANGE_EVENT));

    vi.advanceTimersByTime(DEBOUNCE_MS + 5);
    // We tore down, so the cycle MUST run even though the canonical URL matches.
    expect(onNavigate).toHaveBeenCalled();
    expect(onTransition).toHaveBeenCalledTimes(1);
  });

  it('a same-URL pushState after a settled transition does not tear down again', () => {
    const onNavigate = vi.fn();
    const onTransition = vi.fn();
    uninstall = installSpaHandler({ onNavigate, onTransition });
    ...
    window.dispatchEvent(new CustomEvent(LOCATIONCHANGE_EVENT)); // same URL now
    vi.advanceTimersByTime(DEBOUNCE_MS + 5);
    expect(onNavigate).toHaveBeenCalledTimes(1);
    expect(onTransition).toHaveBeenCalledTimes(1);
  });

  it('a throwing onNavigate does not abort the re-apply cycle', () => {
    const onTransition = vi.fn();
    uninstall = installSpaHandler({
      onNavigate: () => {
