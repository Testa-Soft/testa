...
export function runExperimentCycle(): void {
  ...
  _goals?.teardown();
  _goals = createGoalController({
    track(name, props) {
      track(name, props);
      if (name === 'conversion' && props) {
        const expId = props.experiment_id;
        const goalId = props.goal_id;
        if (typeof expId === 'number' && typeof goalId === 'number') {
          void postLeadConvert(expId, goalId, cookies.getUuid());
        }
      }
    },
  });
  ...
  for (const expConfig of project.experiments) {
    ...
    // Mirror 3.6's POST /api/leads call on variation_applied (deduped per experiment).
    void postLead(expConfig.experiment_id, result.variationId, ctx.visitor.uuidOrSession);
  }
